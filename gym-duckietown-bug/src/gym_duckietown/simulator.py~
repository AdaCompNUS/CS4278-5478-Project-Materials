...
class Simulator(gym.Env):
    ...
    def compute_reward(self, pos, angle, speed):
        ...
        dist_to_stop = ((pos[0] - self.objects[0].pos[0]) ** 2 + (pos[2] - self.objects[0].pos[2]) ** 2) ** 0.5
        #print("dist to stop ", dist_to_stop)
        #print("vel = ", self.speed)
        #print("end compute reward function ")

        # note the speed in the function arg is a constant!
        # should use self.speed instead
        if self.speed > 0.1 and dist_to_stop < 0.5:
            reward = -10.0
        return reward
